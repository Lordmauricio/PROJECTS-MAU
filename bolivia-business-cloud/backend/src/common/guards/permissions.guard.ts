 ...
 * para que revocar un permiso a un rol tenga efecto inmediato sin esperar a
 * que expire el access token. La consulta pasa por `TenantPrismaService`,
 * así que también queda cubierta por RLS: si el `organizationId` del token
 * no es el dueño real de ese `roleId`, la consulta no devuelve nada.
 */
@Injectable()
