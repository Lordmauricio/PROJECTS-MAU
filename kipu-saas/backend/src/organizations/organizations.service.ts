...
export class OrganizationsService {
  ...
  async bootstrapOrganization(input: BootstrapOrganizationInput) {
    ...
    return this.tenantPrisma.run(organizationId, async (tx) => {
      ...
      return { organization, branch, warehouse, posTerminal, membership, roles, user };
    });
  }
  ...
  private async seedDefaultRoles(tx: Prisma.TransactionClient, organizationId: string) {
    ...
    for (const roleKey of DEFAULT_ROLE_KEYS) {
      ...
      const data = grantedKeys
        ...
        .map((permissionId) => ({ organizationId, roleId: role.id, permissionId }));
  ...
  private async getOrCreateFreePlan() {
    const existing = await this.prisma.plan.findUnique({ where: { key: 'free' } });
  ...
  async getSubscription(organizationId: string) {
    return this.tenantPrisma.run(organizationId, (tx) =>
      tx.subscription.findUniqueOrThrow({ where: { organizationId }, include: { plan: true } }),
    );
  }

  /**
   * Métricas reales para el dashboard (sección 9 del prompt). En Fase 1
   * (Foundation) los módulos de ventas/facturación todavía no tienen
   * lógica de negocio, así que estos números legítimamente dan 0 hasta que
   * exista actividad real — nunca se devuelven datos de ejemplo.
   */
  async getDashboardSummary(organizationId: string) {
    ...
    return this.tenantPrisma.run(organizationId, async (tx) => {
      const [
        ...
        tx.sale.aggregate({
          where: { organizationId, status: 'CONFIRMED', createdAt: { gte: startOfDay } },
          _sum: { total: true },
          _count: true,
        }),
        tx.sale.aggregate({
          where: { organizationId, status: 'CONFIRMED', createdAt: { gte: startOfMonth } },
          _sum: { total: true },
          _count: true,
        }),
      ...
      return {
        salesToday: { count: salesToday._count, total: salesToday._sum.total ?? 0 },
